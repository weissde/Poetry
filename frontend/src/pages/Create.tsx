...
export default function CreatePage(): JSX.Element {
  ...
  const contextAppliedRef = useRef<string>("");
  const inquiryPoemId = searchParams.get("poemId") || "";
  const inquiryPoemTitle = searchParams.get("poemTitle") || searchParams.get("topic") || "";
  const inquiryPoemAuthor = searchParams.get("poemAuthor") || "";
  const inquiryPoemContent = searchParams.get("poemContent") || "";
  const inquirySource = searchParams.get("source") || "";
  ...
  useEffect(() => {
    const contextKey = [inquirySource, inquiryPoemId, inquiryPoemTitle, inquiryPoemAuthor].join("|");
    if (!inquiryPoemTitle || contextAppliedRef.current === contextKey) {
      return;
    }
    contextAppliedRef.current = contextKey;

    setReferencePoem(inquiryPoemTitle);
    setActiveTab("review");
    setWorkspaceTab(inquirySource === "learn" ? "creation" : "dialogue");
    if (inquiryPoemContent) {
      setDraftContent(
        `${inquiryPoemContent}\n\n# 迁移练笔\n请仿照《${inquiryPoemTitle}》的意象推进，写 2-4 句自己的表达。`,
      );
      setModernText(
        `刚学完《${inquiryPoemTitle}》，我想把其中的意象、情绪转成自己的生活经验。`,
      );
    }
    setStatusMessage(`已接入《${inquiryPoemTitle}》课堂上下文，可直接做创作迁移。`);
  }, [inquiryPoemAuthor, inquiryPoemContent, inquiryPoemId, inquiryPoemTitle, inquirySource]);
  ...
  const inquiryCard = useMemo(() => {
    if (inquiryPoemTitle && inquiryPoemAuthor) {
      return {
        title: `${inquiryPoemTitle} 探究任务`,
        prompt: `围绕《${inquiryPoemTitle}》展开多轮对话探究，关注意象选择与表达手法。`,
        presetQuestions: [
          `《${inquiryPoemTitle}》中哪一个意象最打动你？为什么？`,
          `${inquiryPoemAuthor}在这首诗中想传达怎样的情感？`,
          `这首诗在艺术手法上有哪些值得借鉴之处？`,
        ],
        completionCta: "完成探究，进入练习",
      };
    }
    return null;
  }, [inquiryPoemTitle, inquiryPoemAuthor]);
