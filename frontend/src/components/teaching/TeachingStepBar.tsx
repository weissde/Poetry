...
export const GLOBAL_TEACHING_STEPS = [
...
export function TeachingStepBarGlobal() {
  ...
  const barStyle = {
    height: isExpanded ? (isTeacherMode ? 56 : 48) : 0,
    ...
    <div style={{ position: 'relative' }}>
      <div style={barStyle}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', height: '100%', fontSize: '13px' }}>
          <span style={{
            color: isTeacherMode ? 'var(--warm-600)' : 'var(--ink-500)',
            fontSize: '12px',
            whiteSpace: 'nowrap',
            marginRight: '4px',
          }}>
            今日目标：{GLOBAL_TEACHING_STEPS[actualCurrentStep]?.note ?? '—'}
          </span>
            ...
            return (
              <div key={step.id} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                {idx < GLOBAL_TEACHING_STEPS.length - 1 && (
                  <span style={{ color: 'var(--neutral)', opacity: 0.5 }}>→</span>
                )}
              </div>
            );
          })}
          {actualCurrentStep >= 0 && actualCurrentStep < GLOBAL_TEACHING_STEPS.length - 1 && (
            <button
              onClick={() => navigate(GLOBAL_TEACHING_STEPS[actualCurrentStep + 1].to)}
              style={{
                display: 'inline-flex',
                alignItems: 'center',
                gap: '4px',
                ...
                borderRadius: '6px',
                border: 'none',
                background: isTeacherMode ? 'var(--warm-500)' : 'var(--brand-ink)',
                color: '#fff',
                fontSize: '13px',
                fontWeight: 500,
                cursor: 'pointer',
