...
function parseLearningReportSections(text: string): {
 ...
 const [summaryTitle = "AI 学情解读", ...summaryBodyLines] = summaryBlock.split("\n");
 const [teacherAdviceTitle = "教师 / 家长建议", ...teacherBodyLines] = teacherBlock.split("\n");
 return {
 summaryTitle: summaryTitle.trim() || "AI 学情解读",
 summaryText: summaryBodyLines.join("\n").trim(),
 teacherAdviceTitle: teacherAdviceTitle.trim() || "教师 / 家长建议",
...
export default function MyLearningPage(): JSX.Element {
 ...
 const title = "诗境通复习提醒";
 const body = `当前还有 ${planTaskStats.pending} 项任务未完成，建议先处理高优先任务。`;
